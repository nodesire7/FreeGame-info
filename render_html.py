...
def render_epic_card(game: Dict[str, Any], variant: str) -> str:
    ...
    summary_html = ""
    if summary:
        # 分别 escape 每个文本项，再拼入 HTML 标签（不要整体 escape）
        escaped_items = [escape_html(item) for item in summary]
        SEP = "</span><span class=\"bg-zinc-800 px-3 py-1\">"
        summary_inner = SEP.join(escaped_items)
        summary_html = f'<div class="flex flex-wrap gap-x-4 gap-y-2 text-[10px] font-bold text-zinc-400 italic"><span class="bg-zinc-800 px-3 py-1">{summary_inner}</span></div>'
    ...
    <div class="lg:w-3/5 flex flex-col justify-between py-2">
        <h4 class="text-2xl lg:text-3xl font-black italic mb-3 lg:mb-4 tracking-tighter uppercase leading-tight">{escape_html(game["title"])}</h4>
        <p class="text-zinc-500 text-xs lg:text-sm leading-relaxed mb-4 lg:mb-6 border-l-4 border-zinc-800 pl-4 line-clamp-2">{escape_html(description)}</p>
        {summary_html}
        <div class="mt-6 lg:mt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 lg:gap-6 border-t-2 border-dashed border-zinc-800 pt-4 lg:pt-6">
            <div class="flex flex-col gap-1">
                <span class="text-xl lg:text-2xl font-black text-red-600 italic leading-tight">{countdown_text}</span>
                <span class="text-[10px] text-zinc-500 font-bold uppercase">{escape_html(deadline_text)}</span>
            </div>
            <a href="{escape_attribute(game["link"])}" class="bg-white text-black px-8 lg:px-12 py-3 lg:py-4 font-black transform -skew-x-12 hover:bg-red-600 hover:text-white transition-all shadow-[6px_6px_0_0_#d10000] text-sm uppercase whitespace-nowrap" target="_blank" rel="noopener noreferrer">{link_text}</a>
        </div>
    </div>
</article>"""
...
def render_steam_card(game: Dict[str, Any]) -> str:
    ...
    <div class="lg:w-3/5 flex flex-col justify-between py-2">
        ...
        <div class="mt-6 lg:mt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 lg:gap-6 border-t-2 border-dashed border-zinc-800 pt-4 lg:pt-6">
            <div class="flex flex-col gap-1">
                <span class="text-xl lg:text-2xl font-black text-red-600 italic leading-tight">{escape_html(price_display)}</span>
                <span class="text-[10px] text-zinc-500 font-bold uppercase">{escape_html(price_detail)}</span>
            </div>
            <a href="{escape_attribute(game["link"])}" class="bg-white text-black px-8 lg:px-12 py-3 lg:py-4 font-black transform -skew-x-12 hover:bg-red-600 hover:text-white transition-all shadow-[6px_6px_0_0_#d10000] text-sm uppercase whitespace-nowrap" target="_blank" rel="noopener noreferrer">立即抢夺</a>
        </div>
    </div>
</article>"""
...
def render_psn_card(game: Dict[str, Any]) -> str:
    ...
    if isinstance(platforms_raw, list):
        platforms_raw = " / ".join(platforms_raw)
    platforms_str = escape_html(str(platforms_raw))
    period_str = escape_html(game.get("period", ""))
    description = (game.get("description") or "当前仍在同步 PlayStation 官方描述。").strip()

    # 组装 meta 行：平台 + 领取时间
    meta_parts = []
    if platforms_str:
        meta_parts.append(f'<span class="bg-zinc-800 px-3 py-1 text-[10px] font-bold text-zinc-400 italic">{platforms_str}</span>')
    if period_str:
        meta_parts.append(f'<span class="bg-zinc-800 px-3 py-1 text-[10px] font-bold text-zinc-400 italic">领取时间：{period_str}</span>')
    meta_html = f'<div class="flex flex-wrap gap-x-4 gap-y-2 mb-4">{("").join(meta_parts)}</div>' if meta_parts else ""
    ...
    <div class="lg:w-3/5 flex flex-col justify-between py-2">
        <div>
            <h4 class="text-2xl lg:text-3xl font-black italic mb-3 lg:mb-4 tracking-tighter uppercase leading-tight">{escape_html(game["title"])}</h4>
            <p class="text-zinc-500 text-xs lg:text-sm leading-relaxed mb-3 lg:mb-4 border-l-4 border-zinc-800 pl-4 line-clamp-2">{escape_html(description)}</p>
            {meta_html}
            <div class="text-lg font-black text-red-600 italic">{highlight}</div>
        </div>
        <div class="mt-6 lg:mt-8 flex items-center justify-end border-t-2 border-dashed border-zinc-800 pt-4 lg:pt-6">
            <a href="{escape_attribute(game["link"])}" class="bg-white text-black px-8 lg:px-12 py-3 lg:py-4 font-black transform -skew-x-12 hover:bg-red-600 hover:text-white transition-all shadow-[6px_6px_0_0_#d10000] text-sm uppercase whitespace-nowrap" target="_blank" rel="noopener noreferrer">立即抢夺</a>
        </div>
    </div>
</article>"""
...
def render_itad_card(game: Dict[str, Any]) -> str:
    ...
    <div class="lg:w-2/5 flex flex-col items-center justify-center gap-3 lg:gap-4 bg-zinc-900 border-[8px] border-white shadow-2xl aspect-video">
    ...
    <div class="lg:w-3/5 flex flex-col justify-between py-2">
        <h4 class="text-2xl lg:text-3xl font-black italic mb-3 lg:mb-4 tracking-tighter uppercase leading-tight">{escape_html(game["title"])}</h4>
        <p class="text-zinc-500 text-xs lg:text-sm leading-relaxed mb-4 lg:mb-6 border-l-4 border-zinc-800 pl-4">ITAD Bundle Giveaway · {escape_html(game_count_text)}</p>
        <div class="mt-6 lg:mt-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 lg:gap-6 border-t-2 border-dashed border-zinc-800 pt-4 lg:pt-6">
            <div class="flex flex-col gap-1">
                <span class="text-xl lg:text-2xl font-black text-red-600 italic leading-tight">{countdown_text}</span>
                <span class="text-[10px] text-zinc-500 font-bold uppercase">截止：{escape_html(expiry_display)}</span>
            </div>
            <a href="{escape_attribute(game["url"])}" class="bg-white text-black px-8 lg:px-12 py-3 lg:py-4 font-black transform -skew-x-12 hover:bg-red-600 hover:text-white transition-all shadow-[6px_6px_0_0_#d10000] text-sm uppercase whitespace-nowrap" target="_blank" rel="noopener noreferrer">ITAD 查看</a>
        </div>
    </div>
</article>"""
